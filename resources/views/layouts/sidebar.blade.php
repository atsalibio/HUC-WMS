...
<aside class="bg-slate-900 text-white w-64 flex-shrink-0 flex flex-col h-full overflow-hidden transition-all duration-300 border-r border-slate-800">
    <div class="h-16 flex items-center px-6 border-b border-slate-800 bg-slate-900">
        <h1 class="text-lg font-extrabold tracking-tight font-display">
            <span class="text-teal-400">ILOILO CITY</span> <span class="text-slate-400">WMS</span>
        </h1>
    </div>

    <div id="sidebar-scroll-container" class="flex-1 overflow-y-auto py-6 scrollbar-hide">
    ...
    <div class="p-4 border-t border-slate-800 bg-slate-900/50 backdrop-blur-md space-y-4">
        <div class="flex items-center p-2 rounded-xl bg-slate-800/30 border border-slate-700/50">
            ...
            <div class="ml-3 overflow-hidden">
                ...
                <p class="text-xs font-semibold text-slate-200 truncate">{{ (optional($user)->FName ?? 'User') . ' ' . (optional($user)->LName ?? '') }}</p>
            </div>
        </div>

<header
    class="sticky top-0 z-40 w-full h-16 bg-white/70 dark:bg-slate-900/70 backdrop-blur-xl border-b border-slate-200 dark:border-slate-800 transition-all duration-300">
            ...
            <div class="hidden lg:flex flex-col">
                ...
                <h2 class="text-sm font-bold text-slate-800 dark:text-white leading-none flex items-center gap-2">
                    ...
                    @if(optional($user)->healthCenter)
                        ...
                        <span
                            class="text-{{ $theme['accent'] }} font-black tracking-tight">{{ $user->healthCenter->Name }}</span>
                    @endif
                </h2>
            </div>
        ...
        <div class="flex items-center gap-2">
            ...
            <div x-data="notificationHandler()" x-init="init()" class="relative">
                <button type="button" @click.stop="open = !open"
                    class="p-2.5 rounded-xl text-slate-400 hover:text-blue-500 hover:bg-slate-100 dark:hover:bg-slate-800 transition-all relative">
                    ...
                    <template x-if="notifications.length > 0">
                        <span
                            class="absolute top-2.5 right-2.5 w-2 h-2 bg-red-500 rounded-full border-2 border-white dark:border-slate-900 group-hover:scale-110 transition-transform pulse-slow"></span>
                    </template>
                </button>
                ...
                <div x-show="open" @click.away="open = false" x-transition:enter="transition ease-out duration-200"
                    ...
                    class="absolute right-0 mt-3 w-80 bg-white dark:bg-slate-800 rounded-2xl shadow-2xl border border-slate-100 dark:border-slate-700 overflow-hidden"
                    x-cloak>
                    <div
                        class="px-5 py-4 border-b border-slate-50 dark:border-slate-700/50 flex justify-between items-center">
                    ...
                    <div class="max-h-96 overflow-y-auto">
                        ...
                        <template x-if="notifications.length === 0">
                            <div class="p-8 text-center">
                                <p class="text-[10px] font-black text-slate-400 uppercase tracking-widest">No new alerts
                                </p>
                            </div>
                        </template>
                    </div>
            ...
            <div class="ml-2 flex items-center gap-3 pl-3 border-l border-slate-100 dark:border-slate-800"
                x-data="{ openProfile: false }">
                <div class="hidden sm:block text-right">
                    <p
                        class="text-xs font-bold text-slate-800 dark:text-white leading-none mb-1 truncate max-w-[120px]">
                ...
                <div @click="openProfile = !openProfile"
                    class="w-10 h-10 rounded-xl flex items-center justify-center text-white font-bold text-sm shadow-lg active:scale-95 transition-all cursor-pointer relative bg-{{ $theme['primary'] }} dark:shadow-{{ $theme['accent'] }}/20">
                    ...
                    <div x-show="openProfile" @click.away="openProfile = false"
                        ...
                        x-transition:enter-start="opacity-0 translate-y-2 scale-95"
                        x-transition:enter-end="opacity-100 translate-y-0 scale-100"
                        class="absolute right-0 top-12 w-48 bg-white dark:bg-slate-800 rounded-2xl shadow-2xl border border-slate-100 dark:border-slate-700 overflow-hidden"
                        x-cloak>
                        <div class="p-2">
                            <form method="POST" action="{{ route('logout') }}">
                                @csrf
                                <button type="submit"
                                    class="w-full px-4 py-3 text-left text-xs font-black text-rose-500 uppercase tracking-widest hover:bg-rose-50 dark:hover:bg-rose-500/10 rounded-xl transition-all flex items-center gap-3">

...
@section('content')
    <div x-data="patientReqManager()" x-init="init()">
        <!-- Header -->
        <div class="mb-14 flex justify-between items-end px-4">
            <div>
                <p class="text-[10px] font-black text-rose-500 uppercase tracking-[0.3em] mb-2">Patient Care</p>
                <h1 class="text-4xl font-black text-slate-800 dark:text-white tracking-tight uppercase">Patient Requisitions
                </h1>
                <p class="text-slate-500 dark:text-slate-400 mt-2.5">Dispense medication and supplies directly to registered
                    patients.</p>
            </div>
            <button @click="openModal()"
                class="px-8 py-4 bg-rose-600 hover:bg-rose-700 text-white font-black text-xs uppercase tracking-widest rounded-3xl shadow-xl shadow-rose-500/20 transition-all active:scale-95 flex items-center gap-3">
        ...
        <div class="grid grid-cols-1 md:grid-cols-4 gap-6 mb-12">
            <div
                class="bg-white dark:bg-slate-800 p-6 rounded-[2rem] border border-slate-100 dark:border-slate-700/50 shadow-sm">
        ...
        <div x-show="showModal" x-transition:enter="transition ease-out duration-300" x-transition:enter-start="opacity-0"
            ...
            <!-- Modal Panel -->
            <div
                class="relative z-10 bg-white dark:bg-slate-800 w-full max-w-4xl rounded-[3rem] shadow-2xl overflow-hidden border border-slate-200 dark:border-slate-700/50 flex flex-col my-auto animate-in zoom-in-95 duration-200">

                <!-- Modal Header (Static) -->
                <div class="px-10 pt-10 pb-6 border-b border-slate-100 dark:border-slate-700/30">
                    <div class="flex justify-between items-center">
                        <div>
                            <p class="text-[10px] font-black text-rose-500 uppercase tracking-[0.2em] mb-1">New Patient
                                Record</p>
                ...
                <div class="flex-1 overflow-y-auto p-10 custom-scrollbar">
                    <form id="requisitionForm" @submit.prevent="submitRequisition" class="space-y-10">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                            <div class="space-y-4">
                                <div class="flex justify-between items-center px-1">
                                    <label
                                        class="text-[10px] font-black text-slate-400 uppercase tracking-widest">Registered
                                        Patient</label>
                                    <span class="text-[10px] font-black text-rose-500 uppercase">System ID</span>
                                </div>
                                <select x-model="formData.patientId"
                                    @change="if(formData.patientId) { formData.patientName = ''; isNewPatient = false; }"
                                    class="w-full px-5 py-4 bg-slate-50 dark:bg-slate-900 border-none rounded-3xl font-bold dark:text-white focus:ring-4 focus:ring-rose-500/10 transition-all">
                                    <option value="">Select Registered Patient...</option>
                                    @foreach($patients as $patient)
                                        <option value="{{ $patient->PatientID }}">{{ $patient->FName }} {{ $patient->LName }}
                                            ({{ $patient->Age }})</option>
                                    @endforeach
                                </select>
                            </div>
                            <div class="space-y-4">
                                <div class="flex justify-between items-center px-1">
                                    <label
                                        class="text-[10px] font-black text-slate-400 uppercase tracking-widest block">Manual
                                        Entry / New Registration</label>
                                    <div class="flex gap-2 items-center">
                                        <input type="checkbox" id="regNew" x-model="isNewPatient"
                                            @change="if(isNewPatient) formData.patientId = ''"
                                            class="w-4 h-4 text-rose-600 rounded">
                                        <label for="regNew"
                                            class="text-[10px] font-black text-slate-400 uppercase pointer-cursor">Register
                                            New</label>
                                    </div>
                                </div>
                                <input type="text" x-model="formData.patientName"
                                    @input="if(formData.patientName && !isNewPatient) formData.patientId = ''"
                                    placeholder="Enter full name for ad-hoc request"
                                    class="w-full px-5 py-4 bg-slate-50 dark:bg-slate-900 border-none rounded-3xl font-bold dark:text-white focus:ring-4 focus:ring-rose-500/10 transition-all">
                            </div>
                        </div>
                        ...
                        <template x-if="isNewPatient">
                            <div
                                class="p-8 bg-rose-50/30 dark:bg-rose-900/10 rounded-[2.5rem] border border-rose-100 dark:border-rose-800 space-y-6 animate-in slide-in-from-top duration-300">
                                <h3 class="text-xs font-black text-rose-500 uppercase tracking-widest px-2">Patient Registry
                                    Details</h3>
                                <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                                    <div class="space-y-2">
                                        <label class="text-[9px] font-black text-slate-400 uppercase ml-1">First
                                            Name</label>
                                        <input type="text" x-model="newPatient.FName"
                                            class="w-full px-4 py-3 bg-white dark:bg-slate-800 border-none rounded-2xl text-xs font-bold dark:text-white">
                                    </div>
                                    <div class="space-y-2">
                                        <label class="text-[9px] font-black text-slate-400 uppercase ml-1">Middle
                                            Name</label>
                                        <input type="text" x-model="newPatient.MName"
                                            class="w-full px-4 py-3 bg-white dark:bg-slate-800 border-none rounded-2xl text-xs font-bold dark:text-white">
                                    </div>
                                    <div class="space-y-2">
                                        <label class="text-[9px] font-black text-slate-400 uppercase ml-1">Last Name</label>
                                        <input type="text" x-model="newPatient.LName"
                                            class="w-full px-4 py-3 bg-white dark:bg-slate-800 border-none rounded-2xl text-xs font-bold dark:text-white">
                                    </div>
                                </div>
                                <div class="grid grid-cols-1 md:grid-cols-4 gap-6">
                                    <div class="space-y-2">
                                        ...
                                        <input type="number" x-model="newPatient.Age"
                                            class="w-full px-4 py-3 bg-white dark:bg-slate-800 border-none rounded-2xl text-xs font-bold dark:text-white">
                                    </div>
                                    <div class="space-y-2">
                                    ...
                                    <div class="md:col-span-2 space-y-2">
                                        <label class="text-[9px] font-black text-slate-400 uppercase ml-1">Contact /
                                            Address</label>
                                        <input type="text" x-model="newPatient.Address"
                                            placeholder="Home address or contact #"
                                            class="w-full px-4 py-3 bg-white dark:bg-slate-800 border-none rounded-2xl text-xs font-bold dark:text-white">
                                    </div>
                                </div>
                            </div>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                            <div class="space-y-4">
                                <label
                                    class="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">Condition /
                                    Diagnosis</label>
                                <input type="text" x-model="formData.diagnosis" required
                                    placeholder="Reason for clinical dispense"
                                    class="w-full px-5 py-4 bg-slate-50 dark:bg-slate-900 border-none rounded-3xl font-bold dark:text-white focus:ring-4 focus:ring-rose-500/10 transition-all">
                            </div>
                            <div class="space-y-4">
                                <label class="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">Clinical
                                    Notes</label>
                                <input type="text" x-model="formData.notes" placeholder="Additional treatment instructions"
                                    class="w-full px-5 py-4 bg-slate-50 dark:bg-slate-900 border-none rounded-3xl font-bold dark:text-white focus:ring-4 focus:ring-rose-500/10 transition-all">
                            </div>
                        </div>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                            <div class="space-y-4">
                                <label class="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">Contact
                                    Info (This Request)</label>
                                <input type="text" x-model="formData.contactInfo"
                                    placeholder="Mobile / Address for follow-up"
                                    class="w-full px-5 py-4 bg-slate-50 dark:bg-slate-900 border-none rounded-3xl font-bold dark:text-white focus:ring-4 focus:ring-rose-500/10 transition-all">
                            </div>
                            <div class="space-y-4">
                                <label class="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">ID Proof
                                    (Photo/Document)</label>
                                <input type="file" @change="formData.idProof = $event.target.files[0]"
                                    class="w-full px-5 py-4 bg-slate-50 dark:bg-slate-900 border-none rounded-3xl font-bold dark:text-white focus:ring-4 focus:ring-rose-500/10 transition-all">
                            </div>
                        </div>
                        ...
                        <div class="space-y-6">
                            <div class="flex justify-between items-center px-2">
                                <h3 class="text-xs font-black text-slate-400 uppercase tracking-[0.2em]">Items to Dispense
                                </h3>
                                <button type="button" @click="addItem()"
                                    class="px-5 py-2.5 bg-slate-900 dark:bg-slate-700 text-white dark:text-slate-300 font-black text-[10px] uppercase tracking-widest rounded-2xl hover:bg-rose-600 transition-all shadow-lg active:scale-95">
                                    + Add Medication
                                </button>
                            </div>
                            ...
                            <div class="grid grid-cols-1 gap-4">
                                <template x-for="(item, index) in formData.items" :key="index">
                                    <div
                                        class="flex flex-col md:flex-row gap-4 items-start md:items-end bg-slate-50 dark:bg-slate-900/40 p-6 rounded-[2.5rem] border border-slate-100 dark:border-slate-800 shadow-sm">
                                        <div class="flex-1 w-full space-y-2">
                                            <label class="text-[10px] font-black text-slate-400 uppercase ml-1">Select
                                                Supply / Medicine</label>
                                            <select x-model="item.itemId" required @change="updateAvailableQty(index)"
                                                class="w-full px-4 py-3 bg-white dark:bg-slate-800 border-none rounded-2xl font-bold dark:text-white text-sm focus:ring-4 focus:ring-rose-500/10">
                                        ...
                                        <div class="w-full md:w-32 space-y-2 text-left">
                                            <label
                                                class="text-[10px] font-black text-slate-400 uppercase ml-1">Available</label>
                                            <div class="px-4 py-3 bg-rose-50 dark:bg-rose-900/10 rounded-xl border border-rose-100 dark:border-rose-800/30 font-black text-rose-700 dark:text-rose-400 text-sm text-center"
                                                x-text="item.availableQty !== null ? item.availableQty : '—'">
                                            </div>
                                        </div>
                                        <div class="w-full md:w-32 space-y-2 text-left">
                                            <label
                                                class="text-[10px] font-black text-slate-400 uppercase ml-1">Quantity</label>
                                            <input type="number" x-model.number="item.quantity" required min="1"
                                                :max="item.availableQty" @input="validateQty(index)"
                                                class="w-full px-4 py-3 bg-white dark:bg-slate-800 border-none rounded-2xl font-black dark:text-white focus:ring-4 focus:ring-rose-500/10"
                                                :class="item.quantity > item.availableQty ? 'ring-2 ring-red-500' : ''">
                                        </div>
                                        <button type="button" @click="removeItem(index)"
                                            class="p-3.5 text-slate-400 hover:text-red-500 hover:bg-white dark:hover:bg-slate-800 rounded-2xl transition-all shadow-sm border border-transparent hover:border-red-100 dark:hover:border-red-900/30">
                                            <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                ...
                <div class="p-10 border-t border-slate-100 dark:border-slate-700/30 bg-slate-50/50 dark:bg-slate-900/20">
                    <div class="flex flex-col sm:flex-row gap-4">
                        ...
                        <button type="submit" form="requisitionForm"
                            class="flex-[1.5] py-5 bg-slate-900 dark:bg-rose-600 hover:bg-slate-800 dark:hover:bg-rose-700 text-white font-black text-xs uppercase tracking-widest rounded-[2.5rem] shadow-2xl shadow-rose-500/20 transition-all active:scale-95">
        ...
        function patientReqManager() {
            return {
                ...
                formData: {
                    ...
                    idProof: '',
                    healthCenterId: '{{ Auth::user()->HealthCenterID ?? 1 }}',
                    items: [{ itemId: '', quantity: 1, availableQty: null }]
                },
                hcStock: @json($hcStock ?? []),
                init() {
                ...
                updateAvailableQty(index) {
                    ...
                    if (item.itemId && this.hcStock[item.itemId]) {
                        item.availableQty = this.hcStock[item.itemId];
                    } else {
                        item.availableQty = 0;
                    }
                },
                ...
                addItem() {
                    this.formData.items.push({ itemId: '', quantity: 1, availableQty: null });
                },
                removeItem(index) {
                ...
                async submitRequisition() {
                    ...
                    for (let item of this.formData.items) {
                        if (item.availableQty !== null && item.quantity > item.availableQty) {
                            alert(`Quantity for item ID ${item.itemId} exceeds available stock!`);
                            return;
                        }
                    }

                    if ((!this.formData.patientId && !this.formData.manualName && !this.isNewPatient) || this.formData.items.length === 0) {

...
<div class="max-w-6xl mx-auto space-y-8" x-data="dpriScanner()">
    ...
    <div class="flex flex-col md:flex-row md:items-center justify-between gap-6">
        <div class="space-y-1">
            <h3 class="text-3xl font-black text-slate-800 dark:text-white mt-1 uppercase tracking-tight">DPRI Intelligent Radar</h3>
            <p class="text-[10px] font-black text-slate-400 uppercase tracking-[0.3em]">Neural Scanning of DOH Drug Price Reference Index</p>
        </div>
        <div class="flex items-center gap-3">
             <a href="https://dpri.doh.gov.ph/download" target="_blank" class="px-6 py-3 bg-white dark:bg-slate-800 border border-slate-100 dark:border-slate-700/50 rounded-2xl text-[10px] font-black text-slate-500 uppercase tracking-widest shadow-sm hover:text-teal-500 transition-all flex items-center gap-2">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-10">
        <div class="lg:col-span-1 space-y-8">
            <div 
                class="relative group cursor-pointer border-2 border-dashed border-slate-200 dark:border-slate-800 rounded-[3rem] p-12 text-center bg-white dark:bg-slate-800 shadow-2xl shadow-slate-200/50 dark:shadow-none hover:border-teal-500/50 transition-all duration-500"
                @dragover.prevent="dragOver = true"
                @dragleave.prevent="dragOver = false"
                @drop.prevent="handleDrop($event)"
                @click="document.getElementById('pdfUpload').click()"
                :class="{'border-teal-500 ring-8 ring-teal-500/5 bg-teal-50/10': dragOver}"
            >
                <input type="file" class="hidden" id="pdfUpload" accept="application/pdf" @change="handleFileSelect($event)">
                
                <div class="space-y-6">
                    <div class="w-24 h-24 mx-auto bg-slate-50 dark:bg-slate-900 rounded-[2rem] flex items-center justify-center text-slate-300 group-hover:text-teal-500 group-hover:bg-teal-50 dark:group-hover:bg-teal-900/20 transition-all duration-500 transform group-hover:scale-110 group-hover:rotate-6 shadow-sm">
            ...
            <div class="bg-slate-900 dark:bg-slate-900/60 rounded-[2.5rem] p-8 space-y-6 shadow-2xl">
                <h4 class="text-[10px] font-black text-teal-400 uppercase tracking-[0.3em] flex items-center gap-3">
        ...
        <div class="lg:col-span-2 space-y-8">
            ...
            <div x-show="isScanning" x-transition class="bg-white dark:bg-slate-800 rounded-[3rem] p-10 border border-slate-100 dark:border-slate-800 shadow-2xl space-y-8">
                <div class="flex items-center justify-between">
                    ...
                    <div class="text-4xl font-black text-teal-500 tabular-nums" x-text="Math.round(totalProgress) + '%'"></div>
                </div>
                <div class="w-full bg-slate-50 dark:bg-slate-900 h-4 rounded-full overflow-hidden p-1 shadow-inner">
                    <div class="h-full bg-slate-900 dark:bg-teal-500 rounded-full transition-all duration-300" :style="'width: ' + totalProgress + '%'"></div>
                </div>
            </div>
                ...
                <div class="px-10 py-8 border-b border-slate-50 dark:border-slate-900/40 bg-slate-50/30 dark:bg-slate-900/10 space-y-6">
                    <div class="flex items-center justify-between gap-6 flex-wrap">
                        <div class="flex items-center gap-4 flex-wrap">
                            <span class="px-4 py-2 bg-teal-50 dark:bg-teal-900/20 text-teal-600 rounded-xl text-[10px] font-black uppercase tracking-widest" x-text="foundItems.length + ' Entities Identified'"></span>
                            <div class="relative group">
                                <span class="absolute inset-y-0 left-4 flex items-center text-slate-300 group-focus-within:text-teal-500 transition-colors">
                                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z"></path></svg>
                                </span>
                                <input type="text" x-model="search" placeholder="Quick search..." class="text-xs font-bold bg-white dark:bg-slate-900 border-none rounded-xl pl-12 pr-4 py-3 w-64 focus:ring-2 focus:ring-teal-500/20 shadow-sm transition-all text-slate-700">
                            </div>
                        </div>
                        <div class="flex items-center gap-4">
                             ...
                             <button @click="confirmImport()" :disabled="isImporting" class="px-8 py-4 bg-slate-900 dark:bg-teal-600 text-white rounded-2xl text-[10px] font-black uppercase tracking-[0.2em] shadow-xl transition-all flex items-center gap-3 active:scale-95">
                        ...
                        <thead class="bg-slate-50/50 dark:bg-slate-900/30 sticky top-0 z-10 border-b border-slate-50 dark:border-slate-800">
                            <tr>
                                <th class="px-10 py-5 text-left">
                                    <input type="checkbox" @change="toggleAll()" :checked="allSelected" class="w-5 h-5 rounded-lg border-none bg-white dark:bg-slate-800 text-teal-500 focus:ring-2 focus:ring-teal-500/10">
                                </th>
                        ...
                        <tbody class="divide-y divide-slate-50/50 dark:divide-slate-800">
                            <template x-for="(item, index) in filteredItems" :key="index">
                                <tr class="hover:bg-slate-50/50 dark:hover:bg-slate-700/20 transition-all group">
                                    <td class="px-10 py-5">
                                        <input type="checkbox" x-model="item.selected" class="w-5 h-5 rounded-lg border-none bg-slate-100 dark:bg-slate-700 text-teal-500 focus:ring-2 focus:ring-teal-500/10 transition-all">
                                    </td>
                                    <td class="px-6 py-5">
